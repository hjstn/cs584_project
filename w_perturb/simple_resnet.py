...
def perturb_model(model, kernel_size, sigma_tensor):
...
if __name__ == '__main__':
    ...
    model.load_state_dict(torch.load('weights/adv1_100'))

    dataloaders, class_names = get_dataloaders(num_features, batch_size, shuffle_datasets, data_dir=data_dir)

    perturbed_model = perturb_model(model, kernel_size, sigma_tensor).to(device)
